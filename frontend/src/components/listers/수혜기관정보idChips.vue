<template>
    <div>
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>수혜기관정보id</v-list-item-title>
                    </v-list-item-content>
                </template>
        ...
        <수혜기관정보id v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import 수혜기관정보id from '../수혜기관정보id.vue';

    export default {
        name: '수혜기관정보idManager',
        components: {
            수혜기관정보id
        },
        props: {
