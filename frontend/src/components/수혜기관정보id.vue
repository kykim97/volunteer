<template>
    <v-card outlined @click="openDialog">
        <v-card-title>
            수혜기관정보 : {{ referenceValue ? referenceValue._links.self.href.split('/').pop() : '' }}
        </v-card-title>

        <v-dialog v-model="pickerDialog" width="500">
            <v-card>
                <v-card-title>수혜기관정보</v-card-title>
                <v-card-text>
                    <수혜기관정보Picker v-model="value" @selected="pick"/>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
        ...
        methods: {
            ...
            openDialog() {
                ...
                if(this.editMode) {
                    ...
                    this.$router.push(path + this.value.수혜기관정보id);
                }
            },
            async pick(val){
                this.newValue = val;
                var path = '/수혜기관정보s';
                var temp = await axios.get(axios.fixUrl(path + '/' + val.수혜기관정보id));
                if(temp.data) {
                    this.referenceValue = temp.data;
                }
